...
# Bayesian Inference ----  
  ...
  inits <- replicate(3, list(pi = rep(0.9, 6),
                             .RNG.name = "base::Mersenne-Twister",
                             .RNG.seed = NA ), simplify = F )
  for(k in 1:3) inits[[k]]$.RNG.seed <- k
  
  m <- read.jagsfile("BayesModel/model_CJS_ver0.R")
...
# Output ----
  ...
  file1 <- paste0("Result/re_modelCJS_ver0_", Sys.Date(), ".csv")
...
# WAIC ----
  ...
  WAIC <- waic(loglik)
  file2 <- paste0("Result/WAIC_modelCJS_ver0_", Sys.Date(), ".csv")
